...
class BobPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bob`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bob WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bob (date, title, link, content)"
